...
```
WLW - agree per-merchant webhook keys - need answer from @security by [date]

Hi Security and Risk teams,

We want to change how the merchant payout webhook is signed. Today all
merchants share one HMAC key (a secret used to prove a message is genuine).
We want one key per merchant instead. This limits the damage if a single key
leaks.
...
Nuance
- Existing merchants keep working during the change. We plan to accept both the
  old and the new key for [number] days.

ACTION REQUIRED
Please copy the questions into your reply and delete the answer that does not apply.

@security — 2 questions:
1. Today one shared HMAC key signs all merchant payout webhooks. Do you agree we
   move to one key per merchant?
   YES / NO —
2. Who owns rotating the per-merchant keys once they exist?
   ANSWER:

@risk — 1 question:
3. The payout flow already runs a velocity check (a limit on how much can move in
   a time window). Can we reuse it as is for this change?
...
required. Lazy consensus does not apply here.
...
1. The first batch is the 3 pilot merchants. The names are on the Confluence
   page:
   [Confluence page link]
...
@risk — 1 question:
1. Does Risk Engineering own building the new limit rule for per-merchant webhook
   keys?
   YES / NO —
...
Shows two things: a resolved Jira key, and an email with no questions. The input
holds no real question, so no ACTION REQUIRED block is invented.
...
```
WLW WHITE-19437 - firewall will alert on missing SDK headers

Hi Mobile and Web SDK teams,

We are adding firewall monitoring for the Wallet SDKs. The firewall (the filter
in front of our services) will start alerting when a request arrives without the
mandatory headers. Alerting only for now. No request will be blocked in this
step.
...
Nuance
- Nothing to change in your code if you already send all three headers. This
  email is only so an alert on your service does not surprise you.
...
subject. Nothing was asked of the reader, so nothing was invented.
