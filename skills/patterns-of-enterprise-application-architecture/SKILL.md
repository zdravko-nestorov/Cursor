...
description: >-
  Enterprise application patterns: layering, domain model vs transaction script, repositories, Unit of Work, DTOs. Use when choosing application architecture patterns or structuring enterprise apps. Prefer over stacking with clean-architecture as equal co-primary.
---
...
For the full rule set, read [reference.md](reference.md) only when the mini guidance is insufficient for a focused architecture audit.
